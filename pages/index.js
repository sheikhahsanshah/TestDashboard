...
function App() {
    ...
    const navigateToStudentDetail = (studentId) => {
        console.log("Navigating to student detail with ID:", studentId)
        // Find the student by ID
        const studentData = studentsData.find((s) => s.id === studentId)

        if (!studentData) {
            console.error("Student not found with ID:", studentId)
            // If student not found, create a default one to avoid the error message
            const defaultStudent = {
                id: studentId || Date.now(),
                name: "New Student",
                status: "Actiu",
                currentCourseLevel: "Not specified",
                objective: "Not defined",
                courses: [],
                attendanceStats: { percentage: 0, averageRating: 0 },
                observations: "",
                nese: "",
                referringTeacher: "",
                incidents: [],
    ...
    return (
        <div className="flex h-screen bg-gray-900">
            ...
            />
            <div
                className={`flex-1 flex flex-col overflow-hidden transition-all duration-300 ease-in-out ${isMainMenuOpen ? "ml-60" : "ml-20"
                    }`}
            >
                <TopBar />
