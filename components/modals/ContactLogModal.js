...
const ContactLogModal = ({ isOpen, onClose, onSave }) => {
  ...
  const handleSaveClick = () => {
    if (!notes.trim()) {
      alert("Si us plau, introdueix les notes del contacte.")
      return
    }
    onSave({ notes })
    ...
    <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4">
      <div ref={modalRef} className="bg-gray-800 rounded-lg shadow-xl w-full max-w-lg p-6 border border-gray-700">
        <div className="flex justify-between items-center mb-4">
          <h2 className="text-xl font-semibold text-white">Nou Contacte Comercial</h2>
          <button onClick={onClose} className="p-1 text-gray-400 hover:text-white rounded-full hover:bg-gray-700">
            <X size={20} />
          </button>
        </div>
        <div>
          <label htmlFor="contactNotes" className="block text-sm font-medium text-gray-300 mb-1">
        ...
        <div className="flex justify-end space-x-3 mt-6">
          ...
          >
            <Save size={16} /> <span>Desar Contacte</span>
          </button>
        </div>
      </div>
    </div>
