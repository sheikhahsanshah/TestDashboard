            ...
            case "seguiment":
                return (
                    <SeguimentTabContent
                        ...
                        onUpdateStudent={handleLocalStudentUpdate}
                    />
                )
            case "gestio_cursos":
...
const SeguimentTabContent = ({ student, openHistoryModal, onUpdateStudent }) => {
    ...
    const handleDayClick = (day) => {
        const dateObj = new Date(currentDisplayDate.getFullYear(), currentDisplayDate.getMonth(), day)
    ...
    const handleSaveCalendarEvent = (eventData) => {
        ...
        const dateStr = `${selectedDay.getFullYear()}-${String(selectedDay.getMonth() + 1).padStart(
            2,
            "0",
        ...
        const newSession = {
            ...
            progressNotes: eventData.description,
        }

        // Add the new session to the student's pastSessions
        const updatedSessions = [...(student.pastSessions || []), newSession]
        onUpdateStudent({ pastSessions: updatedSessions })
                ...
                <div className="grid grid-cols-7 gap-1">
                    ...
                    {Array.from({ length: daysInMonth }).map((_, i) => {
                        ...
                        return (
                            <div
                                key={day}
                                className="h-24 bg-gray-700 rounded p-1.5 text-left relative overflow-hidden flex flex-col cursor-pointer hover:bg-gray-600 transition-colors"
                            ...
                            >
                                ...
                                {session && (
                                    ...
                                    >
                                        {session.title && <p className="font-semibold leading-tight">{session.title}</p>}
                                        ...
                                        {session.type === "exam" && !session.title && (
                                            <p className="font-semibold leading-tight">Examen:</p>
                                        )}
...
const CalendarEventModal = ({ isOpen, onClose, onSave, selectedDay }) => {
    ...
    const formattedDate = selectedDay
        ? selectedDay.toLocaleDateString("ca-ES", { weekday: "long", year: "numeric", month: "long", day: "numeric" })
        : ""
            ...
            <div className="flex items-center justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
                ...
                <div className="inline-block align-bottom bg-gray-900 rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
                    <div className="bg-gray-800 px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                        <div className="sm:flex sm:items-start">
                            ...
                            <div className="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left w-full">
                                <h3 className="text-lg leading-6 font-medium text-white">Afegir Esdeveniment</h3>
                                <div className="mt-1">
                                ...
                                <div className="mt-4">
                                    ...
                                    <input
                                        ...
                                        name="title"
                                        id="title"
                                        className="mt-1 block w-full border border-gray-600 rounded-md shadow-sm py-2 px-3 bg-gray-700 text-gray-200 focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm"
                                        placeholder="Examen, Activitat, etc."
                                        value={title}
                    ...
                    <div className="bg-gray-700 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
                        ...
                        <button
                            type="button"
                            className="mt-3 w-full inline-flex justify-center rounded-md border border-gray-300 shadow-sm px-4 py-2 bg-gray-600 text-base font-medium text-gray-300 hover:bg-gray-500 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 sm:mt-0 sm:ml-3 sm:w-auto sm:text-sm"
                            onClick={onClose}
                        >
                            Cancel·lar
                        </button>
                    ...
                    {expandedCourseId === course.id && (
                        <div className="p-4 bg-gray-700/30">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                                ...
                                {/* Schedule Slots - commented out as requested */}
...
const AvaluacioTabContent = ({ student }) => {
    ...
    const [academyReports, setAcademyReports] = useState([
    ...
    const [academyExams, setAcademyExams] = useState([
    ...
    const [selectedExamType, setSelectedExamType] = useState(null) // 'academy' or 'school'

    const handleDeleteFile = (fileId, reportType) => {
        const confirmDelete = window.confirm("Estàs segur que vols eliminar aquest informe?")
        if (confirmDelete) {
            if (reportType === "academy") {
                setAcademyReports((reports) => reports.filter((report) => report.id !== fileId))
            } else if (reportType === "school") {
                setSchoolReports((reports) => reports.filter((report) => report.id !== fileId))
            }
        }
    }

    const handleOpenFileUploadModal = (reportType) => {
    ...
    const handleFileUploaded = (reportType, newFile) => {
        if (reportType === "academy") {
            setAcademyReports((reports) => [...reports, newFile])
        } else if (reportType === "school") {
            setSchoolReports((reports) => [...reports, newFile])
        }
        setFileUploadModalOpen(false)
        setSelectedReportType(null)
    }

    const handleDeleteExam = (examId, examType) => {
        const confirmDelete = window.confirm("Estàs segur que vols eliminar aquest examen?")
        if (confirmDelete) {
            if (examType === "academy") {
                setAcademyExams((exams) => exams.filter((exam) => exam.id !== examId))
            } else if (examType === "school") {
                setSchoolExams((exams) => exams.filter((exam) => exam.id !== examId))
            }
        }
    }

    const handleOpenExamModal = (examType, exam = null) => {
    ...
    const handleSaveExam = (examType, examData) => {
        if (examType === "academy") {
            if (selectedExam) {
                // Update existing exam
                setAcademyExams((exams) => exams.map((exam) => (exam.id === selectedExam.id ? { ...exam, ...examData } : exam)))
            } else {
                // Add new exam
                setAcademyExams((exams) => [...exams, { ...examData, id: `ae-${Date.now()}` }])
            }
        } else if (examType === "school") {
            if (selectedExam) {
                // Update existing exam
                setSchoolExams((exams) => exams.map((exam) => (exam.id === selectedExam.id ? { ...exam, ...examData } : exam)))
            } else {
                // Add new exam
                setSchoolExams((exams) => [...exams, { ...examData, id: `se-${Date.now()}` }])
            }
        }
    ...
    // Sorting states
    const [academyReportsSortConfig, setAcademyReportsSortConfig] = useState({ columnName: null, direction: "ascending" })
    const [academyExamsSortConfig, setAcademyExamsSortConfig] = useState({ columnName: null, direction: "ascending" })
    const [schoolReportsSortConfig, setSchoolReportsSortConfig] = useState({ columnName: null, direction: "ascending" })
    const [schoolExamsSortConfig, setSchoolExamsSortConfig] = useState({ columnName: null, direction: "ascending" })

    // Sort functions
    const sortData = (data, sortConfig) => {
        if (!sortConfig.columnName) return data

        return [...data].sort((a, b) => {
            const aValue = a[sortConfig.columnName]
            const bValue = b[sortConfig.columnName]
    ...
    // Columns definitions
    const academyReportsColumns = [
        {
            key: "fileName",
            title: "Arxiu",
            render: (report) => (
                <a href={report.url} target="_blank" rel="noopener noreferrer" className="text-blue-400 hover:underline">
                    {report.fileName}
                </a>
            ),
        },
        {
            key: "date",
            title: "Data",
            render: (report) => formatDate(report.date),
        },
    ]

    const academyExamsColumns = [
        { key: "subject", title: "Assignatura" },
        {
            key: "date",
            title: "Data",
        ...
        },
        {
            ...
            title: "Nota",
            render: (exam) => (
                <span className={`font-medium ${exam.grade >= 5 ? "text-green-400" : "text-red-400"}`}>
    ...
    // Apply sorting
    const sortedAcademyReports = sortData(academyReports, academyReportsSortConfig)
    const sortedAcademyExams = sortData(academyExams, academyExamsSortConfig)
    const sortedSchoolReports = sortData(schoolReports, schoolReportsSortConfig)
    const sortedSchoolExams = sortData(schoolExams, schoolExamsSortConfig)
            ...
            <div className="bg-gray-800 p-4 rounded-lg shadow space-y-4">
                ...
                <div>
                    <div className="flex justify-between items-center mb-2">
                        ...
                        <button
                            onClick={() => handleOpenFileUploadModal("academy")}
                    ...
                    <div className="overflow-x-auto max-h-48 scrollbar-thin scrollbar-thumb-gray-600 scrollbar-track-gray-800">
                        <table className="min-w-full divide-y divide-gray-700">
                            <thead className="bg-gray-700/50">
                                <tr>
                                    {academyReportsColumns.map((column) => (
                                        <th
                                            key={column.key}
                                            scope="col"
                                            className="px-4 py-2 text-left text-xs font-medium text-gray-300 uppercase tracking-wider group cursor-pointer"
                                            onClick={() => {
                                                const direction =
                                                    academyReportsSortConfig.columnName === column.key &&
                                                        academyReportsSortConfig.direction === "ascending"
                                                        ? "descending"
                                                        : "ascending"
                                                setAcademyReportsSortConfig({ columnName: column.key, direction })
                                            }}
                                        >
                            ...
                            <tbody className="divide-y divide-gray-700">
                                {sortedAcademyReports.map((report) => (
                                    <tr key={report.id} className="hover:bg-gray-700/50">
                                        {academyReportsColumns.map((column) => (
                                            <td key={column.key} className="px-4 py-2 whitespace-nowrap text-sm">
                                                {column.render ? column.render(report) : report[column.key]}
                                            </td>
                                        ))}
                                        <td className="px-4 py-2 whitespace-nowrap text-right text-sm">
                                            <button
                                                onClick={() => window.open(report.url, "_blank")}
                                                className="text-gray-400 hover:text-white mr-2 p-1 rounded hover:bg-gray-600/50"
                                                title="Descarregar"
                                            >
                                                <Download size={14} />
                                            </button>
                                            <button
                                                onClick={() => handleDeleteFile(report.id, "academy")}
                                                className="text-red-500 hover:text-red-400 p-1 rounded hover:bg-gray-600/50"
                                                title="Eliminar"
                                            >
                                                <Trash2 size={14} />
                                ...
                                {sortedAcademyReports.length === 0 && (
                                    <tr>
                                        <td colSpan={academyReportsColumns.length + 1} className="text-center py-4 text-gray-500 italic">
                                            No hi ha informes.
                                        </td>
                                    </tr>
                                )}
                ...
                <div>
                    <div className="flex justify-between items-center mb-2">
                        ...
                        <button
                            onClick={() => handleOpenExamModal("academy")}
                            className="flex items-center space-x-1 text-xs text-blue-400 hover:text-blue-300 bg-gray-600 px-2 py-1 rounded hover:bg-gray-500"
                        >
                            <Plus size={14} />
                    ...
                    <div className="overflow-x-auto max-h-48 scrollbar-thin scrollbar-thumb-gray-600 scrollbar-track-gray-800">
                        <table className="min-w-full divide-y divide-gray-700">
                            <thead className="bg-gray-700/50">
                                <tr>
                                    {academyExamsColumns.map((column) => (
                                        <th
                                            key={column.key}
                                            scope="col"
                                            className="px-4 py-2 text-left text-xs font-medium text-gray-300 uppercase tracking-wider group cursor-pointer"
                                            onClick={() => {
                                                const direction =
                                                    academyExamsSortConfig.columnName === column.key &&
                                                        academyExamsSortConfig.direction === "ascending"
                                                        ? "descending"
                                                        : "ascending"
                                                setAcademyExamsSortConfig({ columnName: column.key, direction })
                                            }}
                                        >
                                            <div className="flex items-center">
                                                {column.title}
                            ...
                            <tbody className="divide-y divide-gray-700">
                                {sortedAcademyExams.map((exam) => (
                                    <tr key={exam.id} className="hover:bg-gray-700/50">
                                        {academyExamsColumns.map((column) => (
                                            <td key={column.key} className="px-4 py-2 whitespace-nowrap text-sm">
                                                {column.render ? column.render(exam) : exam[column.key]}
                                            </td>
                                        ))}
                                        <td className="px-4 py-2 whitespace-nowrap text-right text-sm">
                                            <button
                                                onClick={() => handleOpenExamModal("academy", exam)}
                                                className="text-indigo-400 hover:text-indigo-300 mr-2 p-1 rounded hover:bg-gray-600/50"
                                                title="Editar"
                                            >
                                                <Pencil size={14} />
                                            ...
                                            <button
                                                onClick={() => handleDeleteExam(exam.id, "academy")}
                                                className="text-red-500 hover:text-red-400 p-1 rounded hover:bg-gray-600/50"
                                                title="Eliminar"
                                            >
                                                <Trash2 size={14} />
                                ...
                                {sortedAcademyExams.length === 0 && (
                                    <tr>
                                        <td colSpan={academyExamsColumns.length + 1} className="text-center py-4 text-gray-500 italic">
                                            No hi ha notes d'exàmens.
                                        </td>
                                    </tr>
                                )}
                            </tbody>
            ...
            <div className="bg-gray-800 p-4 rounded-lg shadow space-y-4">
                ...
                <div>
                    <div className="flex justify-between items-center mb-2">
                        ...
                        <button
                            onClick={() => handleOpenFileUploadModal("school")}
                            className="flex items-center space-x-1 text-xs text-blue-400 hover:text-blue-300 bg-gray-600 px-2 py-1 rounded hover:bg-gray-500"
                        >
                            <Upload size={14} />
                            <span>Pujar Informe</span>
                    ...
                    <div className="overflow-x-auto max-h-48 scrollbar-thin scrollbar-thumb-gray-600 scrollbar-track-gray-800">
                        <table className="min-w-full divide-y divide-gray-700">
                            <thead className="bg-gray-700/50">
                                <tr>
                                    {schoolReportsColumns.map((column) => (
                                        <th
                                            key={column.key}
                                            scope="col"
                                            className="px-4 py-2 text-left text-xs font-medium text-gray-300 uppercase tracking-wider group cursor-pointer"
                                            onClick={() => {
                                                const direction =
                                                    schoolReportsSortConfig.columnName === column.key &&
                                                        schoolReportsSortConfig.direction === "ascending"
                                                        ? "descending"
                                                        : "ascending"
                                                setSchoolReportsSortConfig({ columnName: column.key, direction })
                                            }}
                                        >
                                            <div className="flex items-center">
                                                {column.title}
                            ...
                            <tbody className="divide-y divide-gray-700">
                                {sortedSchoolReports.map((report) => (
                                    <tr key={report.id} className="hover:bg-gray-700/50">
                                        {schoolReportsColumns.map((column) => (
                                            <td key={column.key} className="px-4 py-2 whitespace-nowrap text-sm">
                                                {column.render ? column.render(report) : report[column.key]}
                                        ...
                                        <td className="px-4 py-2 whitespace-nowrap text-right text-sm">
                                            ...
                                            <button
                                                onClick={() => handleDeleteFile(report.id, "school")}
                                                className="text-red-500 hover:text-red-400 p-1 rounded hover:bg-gray-600/50"
                                                title="Eliminar"
                                            >
                                                <Trash2 size={14} />
                                ...
                                {sortedSchoolReports.length === 0 && (
                                    <tr>
                                        <td colSpan={schoolReportsColumns.length + 1} className="text-center py-4 text-gray-500 italic">
                ...
                <div>
                    <div className="flex justify-between items-center mb-2">
                        ...
                        <button
                            onClick={() => handleOpenExamModal("school")}
                            className="flex items-center space-x-1 text-xs text-blue-400 hover:text-blue-300 bg-gray-600 px-2 py-1 rounded hover:bg-gray-500"
                        >
                            <Plus size={14} />
                            <span>Afegir Nota</span>
                                ...
                                <tr>
                                    {schoolExamsColumns.map((column) => (
                                        <th
                                            ...
                                            onClick={() => {
                                                const direction =
                                                    schoolExamsSortConfig.columnName === column.key &&
                                                        schoolExamsSortConfig.direction === "ascending"
                                                        ? "descending"
                                                        : "ascending"
                                                setSchoolExamsSortConfig({ columnName: column.key, direction })
                                            }}
                                        >
                                            <div className="flex items-center">
                                                {column.title}
                            ...
                            <tbody className="divide-y divide-gray-700">
                                {sortedSchoolExams.map((exam) => (
                                    <tr key={exam.id} className="hover:bg-gray-700/50">
                                        ...
                                        <td className="px-4 py-2 whitespace-nowrap text-right text-sm">
                                            <button
                                                onClick={() => handleOpenExamModal("school", exam)}
                                                className="text-indigo-400 hover:text-indigo-300 mr-2 p-1 rounded hover:bg-gray-600/50"
                                                title="Editar"
                                            >
                                                <Pencil size={14} />
                                            ...
                                            <button
                                                onClick={() => handleDeleteExam(exam.id, "school")}
                                                className="text-red-500 hover:text-red-400 p-1 rounded hover:bg-gray-600/50"
                                                title="Eliminar"
                                            >
                                                <Trash2 size={14} />
                                ...
                                {sortedSchoolExams.length === 0 && (
                                    <tr>
                                        <td colSpan={schoolExamsColumns.length + 1} className="text-center py-4 text-gray-500 italic">
...
const FileUploadModal = ({ isOpen, onClose, onFileUploaded, reportType }) => {
    ...
    const validateFile = (file) => {
        ...
        if (!allowedTypes.includes(file.type)) {
            setUploadError("Tipus de fitxer no vàlid. Només .pdf, .doc i .docx.")
            setSelectedFile(null)
            return
        }

        if (file.size > maxSize) {
            setUploadError("El fitxer és massa gran. Màxim 10MB.")
            setSelectedFile(null)
            return
        }
    ...
    const handleUpload = () => {
        if (!selectedFile) {
            setUploadError("Si us plau, selecciona un fitxer.")
            return
        }
        ...
        const interval = setInterval(() => {
            setUploadProgress((prevProgress) => {
                ...
                if (newProgress >= 100) {
                    ...
                    onFileUploaded(reportType, newFile)
                    return 100
                }
                return newProgress
            })
            ...
            <div className="flex items-center justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
                ...
                <div className="inline-block align-bottom bg-gray-900 rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
                    <div className="bg-gray-800 px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                        <div className="sm:flex sm:items-start">
                            <div className="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left w-full">
                                <h3 className="text-lg leading-6 font-medium text-white">Pujar Informe</h3>
                                <div
                                    className="mt-2 p-4 border-2 border-dashed border-gray-600 rounded-md cursor-pointer"
                                    onDrop={handleDrop}
                                ...
                                >
                                    ...
                                    {selectedFile && (
                                        <p className="text-sm text-gray-400 mt-2">Fitxer seleccionat: {selectedFile.name}</p>
                                    )}
                                </div>
                                {uploadError && <p className="text-red-500 text-sm mt-2">{uploadError}</p>}
                                {isUploading && (
                                    <div className="mt-4">
                                        <p className="text-gray-300 text-sm">Pujant...</p>
...
const ExamModal = ({ isOpen, onClose, onSave, exam, examType }) => {
    ...
    const handleSave = () => {
        ...
        if (grade === "" || isNaN(grade) || grade < 0 || grade > 10) {
            alert("La nota ha de ser un número entre 0 i 10.")
            return
        }
            ...
            <div className="flex items-center justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
                ...
                <div className="inline-block align-bottom bg-gray-900 rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
                    <div className="bg-gray-800 px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                        <div className="sm:flex sm:items-start">
                            <div className="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left w-full">
                                <h3 className="text-lg leading-6 font-medium text-white">{exam ? "Editar Examen" : "Afegir Examen"}</h3>
                                <div className="mt-2">
                                    <label htmlFor="date" className="block text-sm font-medium text-gray-300">
                                        Data
                                    </label>
