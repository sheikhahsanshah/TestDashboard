...
const CalendarEventCard = ({ event, onNavigateToTracking, onEventUpdate }) => {
    ...
    const filteredTeachers = docents.filter((docent) =>
        docent.toLowerCase().includes(teacherSearchTerm.toLowerCase())
    )
    ...
    return (
        <div
            ...
            style={{ top: `${topPosition}px`, height: `${height}px`, minHeight: "80px" }}
        >
                ...
                <div className="flex space-x-1">
                    {event.concurrentCourses.map((course) => (
                        <button
                            ...
                            className={`flex-shrink-0 px-2 py-0.5 text-xs rounded-t ${course.active
            ...
            <div className="flex items-center justify-between space-x-2 text-sm flex-shrink-0 mt-auto">
                <div className="relative flex-1 min-w-0" ref={aulaDropdownRef}>
                    ...
                    >
                        ...
                        <ChevronDown size={14} className={`text-gray-400 flex-shrink-0 ml-1 transform transition-transform duration-200 ${isAulaDropdownOpen ? "rotate-180" : ""
                            }`} />
                    </button>
                    {isAulaDropdownOpen && (
                                ...
                                {aulas.map((aula) => (
                                    <li key={aula}>
                                        <a
                                            ...
                                            className={`block px-3 py-1.5 text-sm text-gray-200 hover:bg-gray-500 ${event.aula === aula ? "bg-blue-700 font-semibold" : ""
                                                }`}
                                        >
                                            Aula {aula}
                                        </a>
