...
// Component Targeta Esdeveniment Calendari - Removed Status Dropdown
const CalendarEventCard = ({ event, onNavigateToTracking, onEventUpdate }) => {
  ...
  const handleAulaDropdown = () => console.log("Aula dropdown clicked for session:", event.id)
  const handleCourseTabClick = (courseId) => console.log("Course tab clicked:", courseId)
  ...
  return (
    <div
      ...
      style={{ top: `${topPosition}px`, minHeight: "80px" }}
    >
        ...
        <div className="flex space-x-1">
          {event.concurrentCourses.map((course) => (
            <button
              ...
              className={`flex-shrink-0 px-2 py-0.5 text-xs rounded-t ${course.active ? "bg-gray-800 text-gray-200 font-medium" : "bg-gray-600 text-gray-400 hover:bg-gray-500 hover:text-gray-200"}`}
            >
              {course.name}
            </button>
          ))}
      ...
      <div className="flex items-center justify-between space-x-2 flex-shrink-0">
        <div className="relative flex-1 min-w-0" ref={teacherDropdownRef}>
          ...
          >
            ...
            <ChevronDown
              size={14}
              className={`text-gray-400 flex-shrink-0 ml-1 transform transition-transform duration-200 ${isTeacherDropdownOpen ? "rotate-180" : ""}`}
            />
          </button>
          {isTeacherDropdownOpen && (
            <div className="absolute left-0 mt-1 w-full bg-gray-600 border border-gray-500 rounded-md shadow-lg z-10 max-h-48 overflow-y-auto">
                  ...
                  filteredTeachers.map((docent) => (
                    <li key={docent}>
                      <a
                        ...
                        className={`block px-3 py-1.5 text-sm text-gray-200 hover:bg-gray-500 ${event.teacher === docent ? "bg-blue-700 font-semibold" : ""}`}
                      >
                        {docent}
                      </a>
                    </li>
      ...
      <div className="flex items-center justify-between space-x-2 text-sm flex-shrink-0 mt-auto">
        ...
        {/* Removed Status Dropdown Button */}
