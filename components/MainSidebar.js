"use client"
import { CalendarDays, MapPin, BookOpen, GraduationCap, CreditCard, ChevronsLeft, ChevronsRight } from "lucide-react"

// Updated mainNavItems to include all views
const mainNavItems = [
...
const MainSidebar = ({ activeView, onNavigate, isMainMenuOpen, onToggleMainMenu }) => {
  return (
    ...
    >
      <div
        className={`h-[68px] flex items-center border-b border-gray-700 ${isMainMenuOpen ? "px-6 justify-start" : "justify-center"}`}
      >
        <span
      ...
      <nav className="flex-1 mt-4 px-2 space-y-1 overflow-y-auto overflow-x-hidden">
        {mainNavItems.map((item) => (
          <a
            ...
            title={item.label}
            className={`flex items-center space-x-3 px-3 py-2.5 text-sm rounded-md transition-colors ${
              activeView === item.id
      ...
      <div className="p-2 border-t border-gray-700 flex-shrink-0">
        <button
          ...
          className="w-full flex items-center justify-center p-2 rounded-md text-gray-400 hover:bg-gray-700 hover:text-white transition-colors"
